<template>
  <div>
    ...
    <!-- :custom-filter="customFilter(search)" -->
    ...
    <template v-else>
      ...
      >
        <template v-slot:header class="d-flex">
          <v-text-field v-model="search" clearable flat solo-inverted hide-details label="Buscar"></v-text-field>
          <dialog-add />
        </template>
        ...
        <template v-slot:default="props">
          <v-row>
            <v-col v-for="(item, i) in props.items" :key="i" cols="12">
              <OrderService :call="item" :user_id="item.created_by" />
            </v-col>
          </v-row>
        </template>
